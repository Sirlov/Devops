...
def start(update: Update, context):
	...
	update.message.reply_text(f'Привет {user.full_name}! Я бот_nemo. Введите команду /help для обзора списка доступных команд')

# Функция обзора возможных команд
def helpCommand(update: Update, context):
	commands_list = [ 
		...
		"/get_emails - таблица электронных почт",
		"/get_phone_numbers - таблица телефонных номеров",
		"/get_repl_logs - вывода логов репликации БД"
	] 
	update.message.reply_text("\n".join(commands_list))

# Функции для поиска и записи электронной почты
...
def find_email (update: Update, context):
	...
	update.message.reply_text('Вы хотите сохранить данные? (ДА/НЕТ)')
	context.user_data['emailList'] = emailList
	return 'save_email'	
def save_email(update, context):
	user_response = update.message.text.upper() # Получение ответ пользователя
	if user_response == 'ДА':
		emailList = context.user_data['emailList']
		connection = psycopg2.connect(user=usernameBD,
								password=passwordDB,
								host=hostDB,
								port=portDB,
								database=databaseDB)
		...
		cursor.close()
		connection.close()
		update.message.reply_text('Адреса email сохранены в базе данных')
	elif user_response == 'НЕТ':
		update.message.reply_text('Адреса email не будут сохранены в базе данных')
	else:
		update.message.reply_text('Ответ некорректен. Пожалуста укажите ДА или "НЕТ"')
	return ConversationHandler.END # Завершаем выполнение обработчика

# Функции поиска и записи телефонных номеров
...
def save_phone_number(update, context):
	...
	if user_response == 'ДА':
		...
		for phoneNumber in phoneNumberList:
			cursor.execute("INSERT INTO Phone (\"Номер телефона\") VALUES (%s)", (phoneNumber,))
		connection.commit()
		cursor.close()
		connection.close()
		update.message.reply_text('Телефонные номера сохранены в базе данных')
	elif user_response == 'НЕТ':
...
# Функции проверки пароля на сложность
...
# Функция подключения по SSH к ПЭВМ
...
# Функция сбора информации о запущенных службах
...
# Функция вывода телефонных номеров из таблицы
